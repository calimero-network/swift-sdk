...
2. **Authenticate** with a username and password — the admin credentials the
   node was initialised with. There is no first-login setup code: core
   0.11.0-rc.17 creates the admin at `merod init`.
