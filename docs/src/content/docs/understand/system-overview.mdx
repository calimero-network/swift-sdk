...
<Aside type="note">
  Real-time events (SSE) and blob-streaming polish are on the roadmap (milestones
  M3/M5). Until then, MeroKit covers transport + auth + the full admin/RPC
  surface. For the protocol the node implements, see the
